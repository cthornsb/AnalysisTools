...
PixieLoader::PixieLoader(const char* fname, const char* tname){
	...
	std::cout << " PixieLoader: Successfully loaded " << num_good_branches << " branches\n";
}
...
unsigned short PixieLoader::LoadFile(const char* fname, const char* tname){
	...
	if(!file->IsOpen()){
		std::cout << " Warning! Failed to load input file '" << fname << "'\n";
		file_open = false;
		file->Close();
		delete file;
		return 0;
	...
	if(!tree){
		std::cout << " Warning! Failed to load input tree '" << tname << "'\n";
		file_open = false;
		file->Close();
		delete file;
		return 0;
...
bool PixieLoader::FillEntry(long long entry){
	if(GetEntry(entry)){ return Fill(); }
	return false;
}

bool PixieLoader::SetTree(TTree *output_tree){
	if(!output_tree){ return false; }
	outTree = output_tree;
...
void PixieLoader::SetOutputBranches(std::string input){
	if(input.size() < 8){
		std::cout << " PixieLoader: Failed to set output branches\n";
		return;
	}
	unsigned short count = 0;
	for(unsigned short i = 0; i < 8; i++){
